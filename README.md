...
# ---- 2t58 爬虫：兜底源列表（可选，见 7.8）----
# 上面那组域名**全都**拿不到时才走这一层。只能放同族站点（id 编码规则一致）
# MUSIC_2T58_FALLBACK_BASES=https://www.aat.cx/
# ---- 2t58 爬虫：直连失败后的备用线路（可选，见 7.9）----
# MUSIC_2T58_PROXY_NODES=3
# ---- 2t58 爬虫：回源节流与快速失败（可选，见 7.10）----
# MUSIC_2T58_SOURCE_QPS=2
...
| `MUSIC_2T58_PROXY_NODES` | 某条源直连失败时，一次实时取几条巨量代理节点来试（见 7.9） | `3` |
...
#### 2026-09-26 更新：两个 IP 都进过封禁，临时绑 `.79` 救活了站点

当天实测：**默认出口 `.28` 对两个域名全封**（`www` TCP 丢包、`music` 回 `444`），
而 `.79` 对 `www.2t58.com` 可用（`200`，过验证后拿到 39 KB 真实首页）。
所以这一项临时改成了：

```bash
MUSIC_2T58_SOURCE_IP=118.107.19.79
```

**这只是救急**，不是长期方案：`.79` 迟早也会被封（7.5 的轮封规律没变）。真正的兜底是
7.9 的"直连失败自动走代理"——当两个出口都被封时，请求会自动经巨量代理节点出去，
不用再人工换 IP。所以下次发现站点又空白时，**先看 7.9 的代理线路是否正常**，
不要再急着换 `.28` / `.79`。想彻底不依赖出口 IP，把这一项改回空值即可（默认出口要是被封，
就由 7.9 接手）。
...
### 7.9 备用线路：巨量代理 IP（第一层与兜底源都挂）

某条源**直连**拿不到时，爬虫会换一条线路 —— 实时取一批**巨量代理 IP**，对**同一条源**
再试一遍（页面与 `play.php` 都是）。这是"直连"之外的第二条线路，不是第三层源站。

**触发顺序**（以第一层为例）：

```
域名A 直连 → 失败 → 域名A 走巨量代理（逐节点试）→ 仍失败才把域名A 标记冷却
          → 域名B 直连 → 失败 → 域名B 走巨量代理 → …
          → 第一层全失败 → 兜底源 aat.cx 直连 → 失败 → aat.cx 走巨量代理 → 仍失败才标记冷却
```

**只有两条线路都失败才把该源标记冷却**。代理救回来时**不**冷却 —— 那说明源站本身没问题，
是我们自己的直连出口不通，冷却它只会让接下来的请求白白少一条路。

**为什么第一层也要挂**（2026-09-26 实测）：源站是按**来源 IP** 封的（见 7.5），被封时
两个域名会一起直连不通 —— `www.2t58.com` 在 TCP 层丢包（卡到 connect 超时）、
`music.2t58.com` 直接回 `444`。这种状态下经代理节点访问 `music.2t58.com`
**能正常过验证并拿到真实页面**（当天实测 8 条节点里 3 条可用，返回的首页 39 KB 含真实榜单），
所以代理不是"没意义"，而是直连被封时唯一还能出数据的路。

**已知限制**：代理服务商自己挡了 `www.2t58.com` —— HTTPS 层直接 `403 Forbidden`，
且 `http://example.com/` 经同一代理返回的错误页**一模一样**，说明是代理侧的 ACL、与 2t58 无关。
所以挂代理时通常只有 `music` 那条能成，`www` 那条必然失败，属正常现象。

**代理从哪来**：走平台接口 `GET {XIAOYING_API_BASE}/api/ProxyIp/juliang/proxies`。
巨量的**业务编号 / 密钥 / 代理账密全部由平台侧 `.env` 持有**，调用方**什么都不用填**，
只需平台签名（复用 `Web/services/xiaoying_api.py` 的 `auth_params`）。取不到（平台没配、
被拒、网络不通）时**静默降级**成"本次只走直连"，代理出问题不影响原有的直连行为。

**为什么每次都要实时提取、不缓存**：这是**动态**代理，平台文档明确"节点到期后连接会失败，
需重新提取"。所以每次直连失败都重新提取一批（取回来的节点过期即弃），挨个试，
一条不通就换下一条，而不是逮着同一条反复重试。条数由 `MUSIC_2T58_PROXY_NODES` 控制（默认 3）。

**⚠️ 过验证的整段必须走同一个节点**：源站的人机验证要求 `GET 验证页 → POST 提交 → GET 真实页`
从**同一个出口 IP** 发出去，中途换 IP 会被重新弹验证。所以代码是把"整套流程"交给同一个节点，
节点不成才整体换下一个。
...
| 变量 | 说明 | 默认值 |
|---|---|---|
| `MUSIC_2T58_PROXY_NODES` | 一次实时取几条代理节点来试 | `3` |

### 7.10 回源节流与快速失败（防 502）

**502 是怎么来的**（2026-09-26 复盘）：源站封我们的根因是**回源太密** —— 当天 `/song/`
被请求 1.5 万次、其中 1.3 万个是不同页面、绝大多数来自同一个爬虫 UA，缓存几乎不命中。
出口一封，每次回源都要先等连接超时，4 个 worker 很快被占满，nginx 等不到上游响应头
（默认 60 秒）→ 用户看到的就是 **502 / 504**。所以除了 7.9 的"换条线路"，还必须
让回源**少发一点、别死等、别拖过 nginx 的超时**。三组配置各管一件事：

**① 少发点 —— 出站节流**（`MUSIC_2T58_SOURCE_QPS`，默认 2）

向源站发请求前先领一个令牌（令牌桶，见 `main.py` 的 `_RateLimiter`），平均速率压在
每秒 2 次；领不到令牌就等，等超过 `MUSIC_2T58_THROTTLE_MAX_WAIT`（默认 5 秒）
**直接放弃本次回源**而不是继续排队 —— 排队只会把 worker 占得更久。
...
**经代理时另用更短的读超时**（`MUSIC_2T58_PROXY_READ_TIMEOUT`，默认 6 秒）：实测代理节点里
有一部分是"连得上但不返数据"的半死节点，只能挂到 read 超时才失败 —— 可用的节点响应都在
1-3 秒内，所以 6 秒足够，卡住就赶紧换下一个（否则 3 个节点全卡满 10 秒，实测要 53 秒）。

**③ 别拖过 nginx 的超时 —— 单次回源时间预算**（`MUSIC_2T58_REQUEST_BUDGET`，默认 25 秒）

一个页面（或一次播放信息）的回源，从第一次尝试开始计时，把所有源、所有代理节点全算进去，
超过预算就立刻放弃、**把页面照常渲染出来**（数据可能少一块），而不是无限拖到 nginx 超时。
这也顺带解决了另一个坑：调大 nginx 的 `proxy_read_timeout` 只是把 502 推后，worker 照样被占满。

**"放弃"与"失败"是两回事**（`main.py` 的 `_SourceThrottled`）

被节流拦下、或超了预算，都**不是源站的错**，所以这类"放弃"：

- **不**把源标记冷却（否则接下来每个请求都白白少一条路）；
- **不**写"这个关键词查不到"的负缓存（否则搜索会被误判成全站搜不到）；
- 播放信息被放弃时**不抛错**，只返回空播放地址 —— 页面照常渲染，只是没有播放器，
  不能因为播放信息拿不到就把整页变成维护页。
...
> **调档建议**：源站又被封、站点频繁出现维护页时，说明回源量还是太大，先把
> `MUSIC_2T58_SOURCE_QPS` 降到 `1`、`MUSIC_2T58_REQUEST_BUDGET` 降到 `15`；
> 站点稳定后想更快出数据，再逐步往上调。
